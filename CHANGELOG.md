...
- OSTIR CSV 파싱 안정화:
  - `parse_csv_output()`에서 헤더 정합성을 강화해 `start_codon`, `start_position` 헤더가 모두 없는 결과를
    유효 후보로 처리하지 않도록 수정.
  - 누락 필드가 있는 행은 `skip` 처리하고, 로그/진단을 통해 누락 원인을 추적하기 쉽도록 정리.
  - 사용자 입력 파싱 단계에서 모호한 매핑이 발생하는 빈도를 낮춰 설계/예측 결과의 신뢰도를 개선.
...
- macOS/직접 실행 UX 개선:
  - `app.py`에서 `__main__` 실행 시 1.2초 지연 후 `webbrowser.open`으로 `http://{host}:{port}`를 자동 오픈하도록 추가.
  - 환경변수 `RBS_AUTO_OPEN_BROWSER`(기본: on)를 통해 자동 오픈 비활성화 제어 가능.
  - Flask 진입점 처리 흐름을 유지하면서 브라우저 오픈 실패가 앱 기동을 가로막지 않도록 방어 처리.
